...
Базовый режим агента не использует сторонние модели или сетевые API.
Опциональное ранжирование гипотез (`--llm`) вызывает OpenAI Responses API
через стандартную библиотеку Python. По умолчанию используется снимок
`gpt-4.1-mini-2025-04-14`; модель меняется через `OPENAI_MODEL`.
Это внешний платный сервис, а не включённая в репозиторий модель.
Схема ответа и поддержка модели сверены с
[официальной документацией](https://developers.openai.com/api/docs/guides/structured-outputs).
Дополнительная Python-зависимость для API не требуется.
